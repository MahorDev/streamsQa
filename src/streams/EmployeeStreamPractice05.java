...
/**
 * Exercise 5: Filter employee names that have length greater than 5 characters
 *
 * Separate practice class for independent execution in IntelliJ IDEA.
 */
public class EmployeeStreamPractice05 {
...
List<String> nameLenGt5 = employees.stream()
                .map(Employee::getName)
                .filter(name -> name.trim().length() > 5)
                .toList();
        System.out.println("5. Names with length > 5: " + nameLenGt5);
    }
}
